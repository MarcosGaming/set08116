  //Random for the noise
  float random(vec3 position, int i)
  {
    vec4 seed4 = vec4(position, i);
  ...
  //Bias calculation
  vec3 light_dir = normalize(spot.position - proj_coords);
  float bias = max(0.000005* (1.0 - dot(normal, light_dir)), 0.0000095);
  ...
  for (int i=0;i<64;i++)
  {
	int index = int(64.0*random(position*1000.0,i))%64;
	if ( texture(shadow_map, shadow_tex_coords + poissonDisk[index]/700.0 ).x  <  z + bias )
	{
		visibility-=0.0135;
	}
  }
